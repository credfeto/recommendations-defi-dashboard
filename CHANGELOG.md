...
- Docker runtime stage no longer errors with `sh: husky: not found` when installing production-only dependencies; `--ignore-scripts` is passed to `npm ci --omit=dev` so all lifecycle scripts (including the `prepare` hook that invokes husky) are skipped via npm's own flag rather than a husky-specific env var
