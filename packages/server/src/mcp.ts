...
// ── helpers (mirrors server-fastify.ts) ────────────────────────────────────
...
server.tool('get_pool_types', 'List the available DeFi pool categories that can be queried', {}, async () => {
  const types = getAvailableTypes().map((t) => ({
    id: t.id,
    name: t.name,
...
server.tool(
  ...
  async ({ poolType, limit }) => {
    ...
    const enriched = await Promise.all(
      pools.slice(0, limit).map(async (pool) => ({
        ...pool,
