...
public sealed class ApiCacheServiceTests : TestBase
{
    ...
    [Fact]
    public async Task GetOrFetchAsync_FreshHit_WithNullDeserialized_FallsThroughToFetchAsync()
    {
        CancellationToken cancellationToken = this.CancellationToken();

        ApiCacheRow freshRow = new("null-key", "null", FixedNow - TimeSpan.FromMinutes(30));
        this._database.WithReturn<ApiCacheRow?>(freshRow);
        ...
        string result = await this._cache.GetOrFetchAsync(
            ...
            fetcher: _ =>
            {
                ...
                return ValueTask.FromResult("fetched-value");
            },
            typeInfo: TestJsonContext.Default.String,
            cancellationToken: cancellationToken
        );

        Assert.Equal(expected: "fetched-value", actual: result);
        Assert.Equal(expected: 1, actual: fetchCount);
    }
    ...
    [Fact]
    public Task GetOrFetchAsync_StaleHit_FetcherThrows_NullStaleCached_ExceptionPropagatesAsync()
    {
        ApiCacheRow staleRow = new("null-stale-key", "null", FixedNow - TimeSpan.FromHours(1.5));
        this._database.WithReturn<ApiCacheRow?>(staleRow);
